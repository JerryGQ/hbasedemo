...
object HadoopFileName {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sc = new SparkContext(
      new SparkConf().setAppName("MohuDeleteHBase")//.setMaster("local[2]")
    )
    //设置hbase
    ...
    while (x != null) {
      ...
      RDD.rddToPairRDDFunctions(result).partitionBy(new HashPartitioner(1))
        .saveAsHadoopFile("hdfs://192.168.0.206/test/out",
          classOf[String], classOf[Integer], classOf[RDDMultipleTextOutputFormat])

      x = buf.readLine()

    }

    //sc.textFile("test/out/*")
